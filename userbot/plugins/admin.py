...
NO_ADMIN = "`Sed,I am not an admin here!`"
NO_PERM = "`sed i have no permision to do that!`"
...
async def gspider(gspdr):
    ...
    if gmute(user.id) is False:
        ...
        if reason:
            await gspdr.edit(f"`Globally tapped!`Reason: {reason}")
        else:
            await gspdr.edit("`Globally tapped!`")
